...
//        benchmark.sortLocalDateTimes(config.getInt("benchmarkdatesorters", "n", 100000), config);
    ...
    void benchmarkStringSortersInstrumented(String[] words, int nWords, int nRuns) {
        ...
        if (isConfigBenchmarkStringSorter("mergesort")) {
        	 InstrumentedHelper <String> helper = new InstrumentedHelper("MergeSort", nWords, config);
			 runStringSortBenchmark(words, nWords, nRuns, new MergeSortBasic<>(helper), timeLoggersLinearithmic);
			 System.out.println("Compares= " + helper.getCompares() + " Swaps= " + helper.getSwaps() + " Hits= " + helper.getHits() + " Copies= " + helper.getCopies());
        }
        ...
        if (isConfigBenchmarkStringSorter("quicksortDualPivot")) {
        	InstrumentedHelper <String> helper = new InstrumentedHelper("QuickSortDualPivot", nWords, config);
...
//        doLeipzigBenchmark("zho-simp-tw_web_2014_10K-sentences.txt", 5000, 1000);
    ...
    private void doLeipzigBenchmarkEnglish(int x) {
        String resource = "eng-uk_web_2002_" + (x < 50000 ? "10K-sentences.txt" : x < 200000 ? "100K-sentences.txt" : "1M-words.txt");
        try {
